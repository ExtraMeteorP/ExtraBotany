...
public class ItemCthulhuEye extends ItemRelicAdv implements IManaUsingItem{
	...
	public ItemCthulhuEye() {
		super(LibItemName.CTHULHUEYE);
			...
			if(pos.isValid()) {
				if(pos.dim == world.provider.dimensionId && player instanceof EntityPlayerMP) {
					if(!world.isRemote){
						...
						if(ManaItemHandler.requestManaExact(stack, player, (int) (range*range/48 * (cooldown+1)), true)){
							...
							if(cooldown == 0) cooldown += (int) 200;
							else {
	...
	public void render(ItemStack stack, EntityPlayer player, float partialTicks) {
		...
		for(int seg = 0; seg < SEGMENTS; seg++) {
			...
			ItemRenderer.renderItemIn2D(Tessellator.instance, f1, f2, f, f3, icon.getIconWidth() * 2, icon.getIconHeight() * 2, 1F / 16F);
